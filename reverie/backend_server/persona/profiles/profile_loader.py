...
---------------------
Loads persona profile definitions (e.g., Enneagram-inspired) used to parameterize:
- need weights / initial values
- communication / expressive style
- scratch identity overrides (optional)
"""
...
def load_profiles(path: Optional[str] = None) -> Dict[str, Dict[str, Any]]:
    """
    Returns a mapping of profile_id -> profile dict.
    """
    ...
    # Normalize: ensure each profile contains its id field for convenience.
...
def get_profile(profile_id: str, path: Optional[str] = None) -> Optional[Dict[str, Any]]:
    """
    Fetch a single profile dict by id. Returns None if not found.
    """
...
def profile_style_text(profile: Optional[Dict[str, Any]]) -> str:
    """
    Produce a compact natural-language style hint for prompt conditioning.
    Safe to pass into LLM prompts.
    """
    if not profile:
        return ""
    ...
    # Keep it short; 1-4 lines is plenty.
