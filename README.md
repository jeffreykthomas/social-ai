...
- **Internal monologue and LLMs**: The continuous internal monologue is chain-of-thought style reasoning. The system supports a hybrid LLM setup: a local open-source LLM (optionally RL-tuned) handles policy and external outputs, while internal reasoning can be delegated to an OpenAI GPT-5 API call.
